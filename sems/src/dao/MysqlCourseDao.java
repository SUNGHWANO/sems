...
public class MysqlCourseDao implements CourseDao{
	...
	public void insert(CourseVo course) throws Throwable {
		...
		try {
			...
			stmt = con.prepareStatement(
					"insert SE_COURSE(TITLE, DEST, HOURS) values(?, ?, ?)");
	...
	public List<CourseVo> list(int pageNo, int pageSize) 
		...
		try {
			con = dbConnectionPool.getConnection();
			stmt = con.prepareStatement(
					"select CNO, TITLE from SE_COURSE"
							+ " order by CNO desc"
							+ " limit ?, ?");
	...
	public CourseVo detail(int no) throws Throwable {
		...
		try {
			con = dbConnectionPool.getConnection();
			stmt = con.prepareStatement(
					"select CNO, TITLE, DEST, HOURS from SE_COURSE"
							+ " where CNO=?");
			stmt.setInt(1, no);
			rs = stmt.executeQuery();
	...
	public void delete(int no) throws Throwable {
		...
		try {
			con = dbConnectionPool.getConnection();
			stmt = con.prepareStatement(
					"delete from SE_COURSE where CNO=?"	);
			stmt.setInt(1, no);
			stmt.executeUpdate();
		} catch (Throwable e) {
			throw e;
